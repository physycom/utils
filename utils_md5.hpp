/* Copyright (C) Stefano Sinigardi, Alessandro Fabbri - All Rights Reserved
* Unauthorized copying of this file, via any medium is strictly prohibited
* Proprietary and confidential
* Stefano Sinigardi <stefano.sinigardi@gmail.com>
* Alessandro Fabbri <alle.fabbri@gmail.com>
* September 2016
*/
...
This will give you back an integer representation of the hash. 
You can then turn this into a hex representation 
if you want to pass it around as a string
char md5string[33];
for(int i = 0; i < 16; ++i)
...
#ifndef UTILS_MD5_HPP
#define UTILS_MD5_HPP
...
template<typename T>
std::string make_md5hash(const T &t)
{
	unsigned char digest[16];
	MD5_CTX context;
	MD5_Init(&context);
	MD5_Update(&context, &t, sizeof t);
	MD5_Final(digest, &context);
	char md5string[33];
	for (int i = 0; i < 16; ++i)
		sprintf(&md5string[i * 2], "%02x", (unsigned int)digest[i]);
...
template<>
std::string make_md5hash(const std::string &s)
{
	unsigned char digest[16];
	MD5_CTX context;
	MD5_Init(&context);
